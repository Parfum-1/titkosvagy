// server.js - Teljes értékű Backend szerver a Titkosvagy rendszerhez (Express + Mongoose + JWT + Stripe/Barion váz)
...
// --- ADATBÁZIS SÉMÁK (MODELLEK) ---

const userSchema = new mongoose.Schema({
  displayName: { type: String, required: true },
  email: { type: String, required: true, unique: true },
  passwordHash: { type: String, required: true },
  credits: { type: Number, default: 10 }, // Kezdő kredit
...
// 4. Üzenet küldése (Kredit levonással)
app.post('/api/messages/send', authenticateToken, async (req, res) => {
  try {
    const { recipientId, messageText } = req.body;
    const user = await User.findById(req.user.userId);

    const messageCost = 6; // Alapdíj szabályzat alapján
    ...
    // Itt történne az üzenet mentése az adatbázisba
...
// 5. Zárt kép feloldása (Kredit levonással)
app.post('/api/photos/unlock', authenticateToken, async (req, res) => {
  try {
    const user = await User.findById(req.user.userId);
    const unlockCost = 10; // Zárt kép alapár
